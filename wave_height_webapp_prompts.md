...
## Phase 18: เครื่องมือมาร์กมือ (Manual Peak/Trough Annotation) — มาร์กแค่จุดสูงสุด-ต่ำสุด

```
สร้าง/แก้ไขหน้า src/app/manual-mark/page.tsx ในโปรเจกต์ wave-height-webapp — นี่คือการออกแบบใหม่ทั้งหมด แทนที่ของเดิมที่เคยทำไว้ (ถ้ามีโค้ดเดิมจาก workflow แบบ step ทีละช่วงเวลา ให้เขียนทับ ไม่ต้องพยายามรักษาโครงสร้างเดิมไว้ถ้าขัดกับ design ใหม่นี้)

**เปลี่ยนแนวคิดหลักจากเดิม:** ไม่ต้อง step ทีละช่วงเวลาแล้วพิมพ์ค่าทุกจุด (เดิมช้าและฝืน) เปลี่ยนเป็น: **ผู้ใช้เลื่อนวิดีโอไปมาอย่างอิสระด้วยตาเอง หาตำแหน่งที่ผิวน้ำอยู่จุดสูงสุด (crest) หรือต่ำสุด (trough) ของแต่ละรอบคลื่น แล้วกดปุ่มบันทึกทีละจุดเท่านั้น** — 1 คาบคลื่นต้องการแค่ 2 จุดข้อมูล (ยอด 1 + ท้อง 1) ไม่ใช่สิบกว่าจุดเหมือนเดิม เร็วกว่ามาก

**สำคัญมาก (เหมือนเดิม): หน้านี้ต้องแยกอิสระจากระบบ auto detection ทั้งหมด** ไม่พึ่งพา RulerCalibrationPanel, MeasurementPoint, worker, SurfaceTracker ใด ๆ — reuse ได้เฉพาะ pure logic ที่ไม่เกี่ยวกับ pixel calibration (captureFrameAtTime, waveDataToCSV/downloadCSV)

ส่วนที่ 1 — หน้าตั้งค่าก่อนเริ่ม (ให้สั้นกระชับที่สุด):
- อัปโหลดวิดีโอ
- input "ความถี่คลื่นที่ตั้งไว้ (Hz)" (optional — ใช้เทียบผลตอนสรุปท้ายสุด)
- วาดกรอบ ROI ครอบไม้บรรทัด+บริเวณผิวน้ำ (drag บน canvas เฟรมแรก) — บันทึกเป็น readingROI ใช้ตลอด session (มีปุ่ม "ปรับกรอบใหม่" ระหว่างทำงานได้เผื่อกล้องขยับ)
- ปุ่ม "เริ่มมาร์ก"

ส่วนที่ 2 — หน้าจอมาร์กหลัก (ออกแบบใหม่ทั้งหมด):

Layout (บังคับให้ชัดเจน ไม่ปล่อยให้ AI ตีความเอง เพราะเคยมีปัญหา UI ไม่ดีมาก่อน):
- Layout แบ่งเป็น 2 คอลัมน์บนจอกว้าง (คอลัมน์ซ้ายกว้างกว่า = พื้นที่ควบคุม+preview, คอลัมน์ขวาแคบกว่า = รายการที่บันทึกแล้ว), บนมือถือ/จอแคบ stack เป็นคอลัมน์เดียว (canvas บนสุด, ควบคุมกลาง, รายการล่างสุด)
- คอลัมน์ซ้าย เรียงจากบนลงล่าง:
  1. Canvas แสดงภาพ zoom เข้า readingROI (ขนาดใหญ่ ชัดเจน เป็นจุดสนใจหลักของหน้าจอ) พร้อม CSS filter จาก brightness/contrast slider (ยังคงมีจาก design เดิม)
  2. แถบควบคุมการนำทางวิดีโอใต้ canvas ทันที: scrubber ลากอิสระได้ (input range ผูกกับ video.currentTime เต็มความยาววิดีโอ), ปุ่ม ◀◀ (ถอยทีละเฟรม/เล็กน้อย) และ ▶▶ (เดินหน้าทีละเฟรม/เล็กน้อย) สำหรับ fine-tune ตำแหน่งให้ตรงจุดสูงสุด/ต่ำสุดเป๊ะ ๆ, ปุ่ม Play/Pause พร้อม select ความเร็ว (0.5x/1x/2x/4x) สำหรับเล่นผ่านคร่าว ๆ หาตำแหน่งคลื่นก่อน
  3. ตัวเลขเวลาปัจจุบันแสดงชัดเจน (เช่น "t = 12.4s")
  4. ช่อง input ค่าที่อ่านได้ (cm) — วางไว้กึ่งกลาง ขนาดใหญ่อ่านง่าย
  5. ปุ่มบันทึก 2 ปุ่มใหญ่ชัดเจน วางเคียงข้างกัน ขนาดอย่างน้อย 48px สูง (touch-friendly):
     - "▲ บันทึกจุดสูงสุด (Crest)" สีเขียว
     - "▼ บันทึกจุดต่ำสุด (Trough)" สีแดง/น้ำเงิน (เลือกสีที่ต่างจากเขียวชัดเจน แยกแยะง่ายด้วยตา)
     - กดปุ่มไหนก็ตาม: บันทึก { timeS: currentTime - referenceTimeS (ถ้ามี, ไม่งั้นใช้ currentTime ตรง ๆ), valueCm: ค่าที่กรอก, type: 'crest'|'trough' } เพิ่มเข้า array, แสดง toast/feedback สั้น ๆ ยืนยันการบันทึก (เช่น "บันทึกแล้ว: t=12.4s, 8.3cm (Crest)" หายไปเองใน 2 วินาที), เคลียร์ช่อง input ค่า (แต่**ไม่ auto-advance เวลา** เพราะผู้ใช้ต้องเลื่อนหาจุดต่อไปเอง ต่างจากโหมด step เดิม)
  6. กราฟเล็ก ๆ ด้านล่างสุดของคอลัมน์นี้ (recharts LineChart ธรรมดา เส้นเดียว ต่อจุด crest/trough ที่บันทึกแล้วเรียงตามเวลา) ให้เห็น pattern คลื่นที่กำลังมาร์กคร่าว ๆ ระหว่างทำ

- คอลัมน์ขวา: รายการจุดที่บันทึกแล้วทั้งหมด **เรียงตามเวลาเสมอ** (ไม่ใช่ตามลำดับที่กด ถ้าผู้ใช้ย้อนกลับไปมาร์กจุดที่เวลาน้อยกว่าทีหลัง ต้องขึ้นมาเรียงอยู่ตำแหน่งที่ถูกต้องอัตโนมัติ) แต่ละแถวแสดง: ไอคอน/สีบอก crest หรือ trough, เวลา, ค่า, ปุ่มลบ (🗑) ท้ายแถว — คลิกที่แถวเพื่อ seek วิดีโอกลับไปเวลานั้นทันที (สะดวกเวลาจะแก้ไข/ตรวจสอบจุดเก่า)
- ปุ่ม "■ หยุดมาร์ก / ดูผลลัพธ์" อยู่ตำแหน่งเห็นชัดเจนเสมอ (เช่น sticky อยู่มุมบนขวาตลอดเวลา ไม่ต้อง scroll หา)

ส่วนที่ 3 — เขียน src/lib/extremaStats.ts (ไฟล์ใหม่) แทนที่การใช้ computeWaveStatistics เดิม (ซึ่งออกแบบมาสำหรับ time series สม่ำเสมอ ใช้กับข้อมูลแบบ crest/trough เท่านั้นไม่ได้):

export interface ExtremaPoint { timeS: number; valueCm: number; type: 'crest' | 'trough'; }
...
export interface ExtremaStats {
  ...
  hSignificant: number;
  periodMeanS: number;
  waveHeights: { heightCm: number; startTimeS: number; endTimeS: number }[];
  warnings: string[];   // เช่น "พบจุดประเภทเดียวกันติดกัน 2 จุด (ควรสลับ crest-trough เสมอ)"
}

export function computeExtremaStats(extrema: ExtremaPoint[]): ExtremaStats
ขั้นตอน:
1. เรียง extrema ตาม timeS
2. ตรวจสอบว่าสลับ crest-trough ถูกต้อง (ห้ามมี type เดียวกันติดกัน 2 จุด) — ถ้าเจอ ไม่ throw error แต่เก็บเป็น warning message ใส่ใน warnings array (ผู้ใช้ควรได้เห็นแต่ไม่ต้อง block การคำนวณ)
3. จับคู่ extrema ที่ติดกัน (adjacent pair ไม่ว่าจะเริ่มด้วย crest หรือ trough) คำนวณ heightCm = |pair[1].valueCm - pair[0].valueCm| เก็บ startTimeS/endTimeS ของคู่นั้นไว้ด้วย
4. hMax = ค่าสูงสุดของ waveHeights, hMean = ค่าเฉลี่ย, hSignificant = ค่าเฉลี่ยของ 1/3 ที่สูงที่สุด (ปัดขึ้นถ้าหารไม่ลงตัว เหมือน convention เดิมจาก Phase 4)
5. คำนวณคาบ: หา timeS ของจุดประเภทเดียวกันที่ติดกัน (เช่น crest กับ crest ถัดไป ข้าม trough ตรงกลาง) คำนวณผลต่างเวลา เก็บทั้งจาก crest-to-crest และ trough-to-trough รวมกันเป็น periodMeanS (ค่าเฉลี่ยทั้งหมด)
6. nWaves = จำนวน waveHeights ที่คำนวณได้

ส่วนที่ 3b — Sine Wave Fitting (คำนวณเส้นโค้ง sine wave อัตโนมัติจากจุดที่มาร์ก):

เพิ่มในไฟล์ src/lib/extremaStats.ts (หรือแยกไฟล์ src/lib/sineFit.ts ถ้าทำให้โค้ดอ่านง่ายกว่า):
...
export interface SineFitResult {
  ...
  periodS: number;
  phaseRad: number;
  offsetCm: number;      // ระดับน้ำนิ่งเฉลี่ยที่ fit ได้ (ค่ากลางของคลื่น)
  rSquared: number;       // ความเข้ากันของ fit กับข้อมูลจริง (0-1, ยิ่งใกล้ 1 ยิ่งเป็น sine บริสุทธิ์)
}

export function fitSineWave(
  extrema: ExtremaPoint[],
  initialFrequencyHz?: number,     // ใช้ความถี่ที่ผู้ใช้ตั้งไว้ตอนต้น (ถ้ามี) เป็นจุดเริ่มค้นหา ช่วยให้ fit แม่นและเร็วขึ้น
  searchRangeFactor: number = 0.4  // ค้นหาความถี่ในช่วง ±40% รอบค่าเริ่มต้น
): SineFitResult | null

อัลกอริทึม (ไม่ต้องพึ่ง library ภายนอก เขียนเองได้ด้วยคณิตศาสตร์พื้นฐาน):
1. ถ้ามีจุดข้อมูลน้อยกว่า 4 จุด: คืนค่า null (fit ไม่ได้ ข้อมูลน้อยเกินไป) — ผู้เรียกต้องจัดการกรณีนี้ (ไม่แสดงผล fit ถ้าเป็น null พร้อมข้อความ "ต้องการอย่างน้อย 4 จุดข้อมูล (2 คาบ) ถึงจะคำนวณ sine fit ได้")
2. ถ้าไม่มี initialFrequencyHz: ประมาณจาก periodMeanS ที่ได้จาก computeExtremaStats (adjacent pairing แบบเดิม) แปลงเป็นความถี่เริ่มต้น
3. Grid search ความถี่ f ในช่วง [f0*(1-searchRangeFactor), f0*(1+searchRangeFactor)] (เช่น สุ่มเช็ค 200 ค่าในช่วงนี้แบบเส้นตรง):
   - สำหรับแต่ละ candidate f: solve linear least squares หาค่า a, b, offset จากสมการ v(t) = offset + a·cos(2πft) + b·sin(2πft) — นี่คือ linear regression 3 ตัวแปรธรรมดา (แก้ระบบสมการ normal equations 3x3 ด้วยมือ ไม่ซับซ้อน เพราะ f ถูกกำหนดตายตัวในแต่ละรอบค้นหาแล้ว)
   - คำนวณ residual sum of squares (SS_res) ของ candidate นี้
4. เลือก f ที่ให้ SS_res ต่ำสุดในบรรดาที่ลองทั้งหมด เป็นคำตอบสุดท้าย
5. แปลงผลลัพธ์: amplitudeCm = √(a²+b²), phaseRad = atan2(b,a) (ให้สอดคล้องกับ v(t) = offsetCm + amplitudeCm·sin(2π·frequencyHz·t + phaseRad)), periodS = 1/frequencyHz
6. rSquared = 1 - SS_res/SS_tot (SS_tot คำนวณจากผลต่างของค่าจริงกับค่าเฉลี่ยรวม)
...
- สุ่มตัวอย่างจากสมการ fit ที่ resolution ละเอียด (numPoints จุดกระจายสม่ำเสมอตลอดช่วงเวลา) สำหรับวาดเป็นเส้นโค้งเรียบทับบนกราฟจุดข้อมูลจริง

อัปเดต UI ที่เกี่ยวข้อง:
- **กราฟ live ระหว่างมาร์ก (ส่วนที่ 2 ข้อ 6):** เมื่อมีจุดข้อมูลครบ 4 จุดขึ้นไป ให้เรียก fitSineWave + generateSineFitCurve แล้ว overlay เส้นโค้ง sine (เส้นประสีเทาอ่อน บางกว่าเส้น/จุดข้อมูลจริง) ทับบนกราฟจุด crest/trough แบบ real-time อัปเดตทุกครั้งที่มีจุดใหม่ — เห็นเส้นโค้งค่อย ๆ นิ่งและแม่นขึ้นเรื่อย ๆ ระหว่างมาร์ก เป็น feedback ที่ดีมาก
- **หน้าสรุปผล (ส่วนที่ 4):** แสดงตาราง sine fit แยกจากตาราง pairing เดิม (สองวิธีเสริมกัน ไม่ใช่แทนที่): amplitudeCm, periodS, frequencyHz, rSquared — ถ้ามีความถี่ที่ตั้งไว้ตอนต้น ให้เทียบ frequencyHz จาก fit กับค่าที่ตั้งไว้ (% ผลต่าง) เหมือนที่ทำกับ periodMeanS จาก pairing method
- ถ้า rSquared < 0.8 แสดงคำเตือนสีเหลืองใต้ตาราง sine fit: "รูปคลื่นที่มาร์กอาจไม่ใช่ sine wave บริสุทธิ์ (R² ต่ำ) ผลจาก sine fit อาจไม่แม่นยำนัก แนะนำดูกราฟเปรียบเทียบด้วยตาก่อนเชื่อตัวเลขนี้"
- กราฟสรุปผลเต็มรูปแบบ overlay เส้นโค้ง sine fit (จาก generateSineFitCurve) ทับบนจุด crest/trough จริง ให้เห็นเทียบกันชัดเจนด้วยตาว่า fit เข้ากับข้อมูลจริงแค่ไหน

เพิ่ม unit test ในไฟล์เดิม (หรือไฟล์ sineFit.ts ถ้าแยก):
- สร้างข้อมูล extrema สังเคราะห์จาก sine wave ที่รู้พารามิเตอร์แน่นอน (เช่น amplitude=8cm, frequency=0.4Hz, phase=0, offset=0 — คำนวณตำแหน่งเวลาและค่าของ crest/trough แต่ละจุดตามสมการ sine ตรง ๆ ไม่ต้องมี noise) → test ว่า fitSineWave คืนค่า amplitudeCm/frequencyHz/offsetCm ใกล้เคียงค่าจริงมาก (error ไม่เกิน 2%) และ rSquared ใกล้ 1.0 (เช่น > 0.99)
- test เดียวกันแต่เพิ่ม noise เล็กน้อยในค่า valueCm (จำลอง error การอ่านค่าด้วยตา) → error ของ fit ยังอยู่ในเกณฑ์ยอมรับได้ (ไม่เกิน 10%) และ rSquared ลดลงตามสัดส่วน noise ที่ใส่ (ยืนยันว่า metric นี้สะท้อนคุณภาพข้อมูลได้จริง)
- test ว่าข้อมูลน้อยกว่า 4 จุดคืนค่า null ไม่ throw error
- test generateSineFitCurve คืนจำนวนจุดตรงตาม numPoints และค่าที่ตำแหน่งเวลาที่รู้คำตอบล่วงหน้า (เช่นตำแหน่ง crest ที่แน่นอนของ fit) ตรงกับที่คำนวณมือได้


- เรียก computeExtremaStats แสดงผล nWaves, hMax, hMean, hSignificant, periodMeanS
- ถ้ามี warnings ให้แสดงเป็นกล่องเตือนสีเหลืองชัดเจน พร้อมบอกว่าจุดไหน (เวลาไหน) ที่ผิดปกติ ให้ผู้ใช้กลับไปแก้ได้ตรงจุด
- ถ้ากรอกความถี่ที่ตั้งไว้ตอนต้น: แสดง "ความถี่ที่ตั้งไว้ vs ความถี่ที่วัดได้ (1/periodMeanS)" พร้อม % ผลต่าง
- กราฟเต็มรูปแบบแสดงจุด crest (สีเขียว)/trough (สีแดง) เชื่อมเส้นตามลำดับเวลา
- ปุ่มดาวน์โหลด CSV (คอลัมน์: time_s, value_cm, type)
- ปุ่ม "มาร์กต่อ" กลับไปหน้ามาร์กหลักโดยไม่รีเซ็ตข้อมูลเดิม

ส่วนที่ 5 — เครื่องมือช่วยอ่านค่า (เหมือนที่ตกลงไว้ก่อนหน้า ยังคงต้องมี):
- Brightness/Contrast slider (CSS filter บน canvas, ช่วง 50%-200%, default 100%)

ส่วนที่ 6 — ข้อกำหนดคุณภาพ UI (บังคับ ไม่ใช่ optional เพราะเวอร์ชันก่อนหน้ามีปัญหา UI มาก่อน):
- ก่อนเขียนโค้ด UI ส่วนนี้ ให้อ่าน /mnt/skills/public/frontend-design/SKILL.md ก่อนเสมอ แล้วทำตามแนวทางเรื่อง spacing, typography, สี ที่ระบุไว้ในนั้น
- ปุ่มทุกปุ่มต้องมี hover/active state ชัดเจน, ขนาดอย่างน้อย 44x44px สำหรับปุ่มสำคัญ (บันทึก crest/trough)
- ห้ามใช้สีเขียว/แดงเป็นสีเดียวในการแยก crest/trough เท่านั้น (คนตาบอดสีบางกลุ่มแยกไม่ออก) ให้มี icon/รูปทรงต่างกันด้วยเสมอ (เช่น ▲ กับ ▼)
- ทดสอบ layout บนหน้าจอกว้าง (desktop) และแคบ (mobile/tablet) ทั้งคู่ก่อนถือว่าเฟสนี้เสร็จ ไม่ใช่ทดสอบแค่จอเดียว

ส่วนที่ 7 — UX เพิ่มเติม (สำคัญพอ ๆ กับ UI เพราะงานนี้ต้องทำซ้ำนาน ๆ):

1. **Keyboard shortcuts** — อย่าให้ผู้ใช้ต้องใช้เมาส์คลิกทุกครั้ง เพิ่ม global keyboard listener (ปิดการทำงานถ้า focus อยู่ในช่อง input ตัวเลข เพื่อไม่ชนกับการพิมพ์ค่า):
   - กด `C` = บันทึกจุดสูงสุด (เทียบเท่าคลิกปุ่ม Crest) — ใช้ค่าที่พิมพ์ไว้ในช่อง input ขณะนั้น
   - กด `T` = บันทึกจุดต่ำสุด (Trough)
   - กด `←` / `→` = ถอย/เดินหน้าทีละเฟรมเล็กน้อย (เหมือนปุ่ม ◀◀/▶▶)
   - กด `Space` = play/pause วิดีโอ
   - กด `Ctrl+Z` (หรือ Cmd+Z บน Mac) = undo การบันทึกล่าสุด (ลบรายการสุดท้ายออกจาก array)
   - แสดง cheat sheet คีย์ลัดเล็ก ๆ มุมหนึ่งของหน้าจอ (พับ/กางได้ ไม่บังพื้นที่หลัก)

2. **ป้องกันข้อมูลหาย** — งานนี้ใช้เวลาทำนาน ถ้าข้อมูลหายเพราะเผลอปิดแท็บจะเสียเวลามาก:
   - เพิ่ม `beforeunload` event listener เตือนก่อนออกจากหน้า/ปิดแท็บถ้ามีข้อมูลที่ยังไม่ได้ดาวน์โหลด CSV (มาตรฐาน browser confirm dialog "การเปลี่ยนแปลงอาจไม่ถูกบันทึก")
   - Auto-save รายการที่บันทึกไว้ลง localStorage ทุกครั้งที่มีการเพิ่ม/ลบจุด (key เช่น `manual-mark-draft`) — ตอนเปิดหน้านี้ใหม่ ถ้าเจอ draft เก่าใน localStorage ให้ถามผู้ใช้ว่าจะ "กู้คืนข้อมูลที่มาร์กค้างไว้" หรือ "เริ่มใหม่" (ไม่ auto-restore เงียบ ๆ เพราะอาจสร้างความสับสนถ้าผู้ใช้ตั้งใจเริ่มวิดีโอใหม่จริง ๆ)

3. **Live stats preview ระหว่างมาร์ก** — ไม่ต้องรอกด "หยุดมาร์ก" ถึงจะเห็นผล ให้แสดงตัวเลขสรุปคร่าว ๆ (nWaves, hMean ปัจจุบัน) เป็นแถบเล็ก ๆ อัปเดตสดอยู่ใต้กราฟตลอดเวลาที่มาร์ก เพื่อให้ผู้ใช้รู้สึกว่ากำลังคืบหน้า และสังเกตความผิดปกติได้ทันที (เช่น hMean สูงผิดคาดมาก อาจพิมพ์ค่าผิดไปแล้ว) โดยไม่ต้องรอจบงานก่อนถึงจะรู้

4. **Input validation ป้องกัน error พื้นฐาน** — ปุ่ม Crest/Trough ต้อง disabled (จาง ๆ กดไม่ได้) ถ้าช่อง input ค่ายังว่างหรือไม่ใช่ตัวเลขที่ถูกต้อง กัน submit ค่าว่าง/ผิดพลาดเข้าไปปนในข้อมูล

5. **Empty state ที่เป็นมิตร** — ตอนยังไม่มีจุดไหนถูกบันทึกเลย คอลัมน์ขวา (รายการจุด) อย่าปล่อยว่างเฉย ๆ ให้แสดงข้อความ/ภาพประกอบสั้น ๆ อธิบายวิธีเริ่มต้น (เช่น "เลื่อนวิดีโอหาจุดสูงสุด/ต่ำสุดของคลื่น พิมพ์ค่าที่อ่านได้ แล้วกด C หรือ T")

ไม่ต้องเขียน unit test สำหรับส่วนนี้ (เป็น UX/interaction ล้วน ๆ เทสอัตโนมัติคุมยาก) แต่เพิ่มในเช็คลิสต์ทดสอบด้วยมือ:
- ทดสอบคีย์ลัดทุกตัวทำงานถูกต้อง โดยเฉพาะต้อง**ไม่ทำงานตอน focus อยู่ในช่อง input ตัวเลข** (ลองพิมพ์ตัวเลขที่มี "c" หรือ "t" ปนไม่ได้เพราะเป็นตัวเลขล้วนอยู่แล้ว แต่ให้เทส Ctrl+Z ตอน focus input ว่าไม่ไปลบรายการโดยไม่ตั้งใจระหว่างพิมพ์)
- ปิดแท็บระหว่างมีข้อมูลค้างอยู่ ตรวจว่าเจอ browser warning จริง
- เปิดหน้าใหม่หลังปิดไปตอนมีข้อมูลค้าง ตรวจว่าเจอ prompt กู้คืนข้อมูลจาก localStorage
- ตรวจว่าปุ่ม Crest/Trough กดไม่ได้จริงตอนช่อง input ว่าง


- test computeExtremaStats กับข้อมูล crest/trough สลับกันถูกต้อง 5-6 คู่ ที่คำนวณมือได้ล่วงหน้า → ตรวจ hMax/hMean/hSignificant/periodMeanS ตรงตามที่คำนวณมือ
- test warning เมื่อมี type ซ้ำติดกัน 2 จุด (ไม่ throw error แค่ใส่ warning)
- test เรียงลำดับ extrema ตามเวลาถูกต้องแม้ input array ไม่เรียงมา

รันเทสด้วย `npm run test` และทดสอบด้วยมือ:
- ลองเลื่อนหาจุดสูงสุด/ต่ำสุดจริงในวิดีโอทดสอบ กดบันทึกสัก 8-10 จุด (4-5 คาบ) ตรวจว่ารายการด้านขวาเรียงเวลาถูกต้อง คลิกแถวแล้ว seek กลับไปถูกเวลาจริง
- ลองลบจุดกลางรายการ ตรวจว่ากราฟ/สถิติอัปเดตถูกต้อง
- ดูหน้าสรุปผล ตรวจตัวเลขสมเหตุสมผล เทียบกับความถี่ที่ตั้งไว้ถ้ามี
- ทดสอบบนหน้าจอมือถือ/จอแคบด้วย ไม่ใช่แค่ desktop
```

**เกณฑ์ผ่านเฟส:** unit test ของ extremaStats.ts และ sine fit ผ่านทั้งหมด (โดยเฉพาะเทส fit กับข้อมูล sine บริสุทธิ์ที่ error ต้องไม่เกิน 2%), ทดสอบด้วยมือมาร์กจุดจริงได้ลื่นไม่สับสน เห็นเส้นโค้ง sine wave ขึ้นทับกราฟจริงและดูสมเหตุสมผลด้วยตา, เลย์เอาต์ใช้งานได้ดีทั้งจอกว้างและจอแคบ, ปุ่ม crest/trough แยกแยะง่ายด้วยตาทันทีไม่ต้องอ่านตัวหนังสือ
